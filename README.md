...
(`settlement_idem_key_v1`) by the summary's `schema_version` (VER-02), so
genuine historical evidence is never reported as tampered. **Cross-pack
chains** verify via the new `prior_after_hash` argument on
`verify_proof_chain` / `AuditBundle` — pass the previous pack's tail
`after_hash` when verifying a later pack in a tenant's chain (the platform
seals packs in series). **Signed exports** (`?export=true` metering/settlement
envelopes) verify offline via `verify_signed_export`. See
...
4. **Settlement is a pure projection of metering.** Same logic — a
   share table, a gross price per unit, a deterministic integer split
   (floor per share, residual absorbed by the largest-share line). The
...
```bash
pip install ./enfinitos_sdk_auditor-0.0.4-py3-none-any.whl --no-deps
...
- `verify_proof_chain(records, prior_after_hash=None) → ChainAuditReport`
...
Walks records in order, asserts link continuity and issued_at
ordering. The first record's `before_hash` must equal
`prior_after_hash` — pass the previous pack's tail `after_hash` for a
later pack in a tenant's chain (cross-pack continuity); omit it for a
standalone/first pack, which enforces the genesis-null invariant.
...
Version-aware idemKey reconstruction (VER-02): `settlement.v2` uses the
3-field content hash, legacy `settlement.v1` the 2-field hash — both
verify.

### `verify_signed_export(export, keys, verifier=None) → SignedExportAuditReport`

Verifies an `export.v1` signed metering/settlement export offline:
envelope checks, key lookup at `exported_at`, payload
re-canonicalisation, transparency-hash check
(`EXPORT_PAYLOAD_HASH_MISMATCH`), and Ed25519 verification over
`f"{payloadCanonical}|{keyId}"`. Accepts a `SignedExport` or the raw
wire dict.
